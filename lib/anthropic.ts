...
export async function getCoachingResponse(userId: string, userMessage: string, conversationHistory: Array<{role:string;content:string}>, dealContext?: string): Promise<ReadableStream> {
  ...
  const fullSystemPrompt = [systemPrompt, userContext ? `\n\n=== CRITICAL: USER PROFILE (ALREADY REGISTERED) ===\nThe user has ALREADY provided all this information during registration. NEVER ask for their name, industry, experience, products, customer types, or competitors. Use this data directly.\n\nRULES:\n1. ALWAYS address the user by their FIRST NAME in your opening line (e.g., 'Great question, Bhadresh!' or 'Bhadresh, let me walk you through this.')\n2. ADAPT your tone based on their experience: 0-5 years = encouraging and energetic, 6-15 years = respectful peer, 16+ years = deferential and collaborative\n3. Use their specific industry and products in examples — do NOT use generic examples\n\nPROFILE DATA:\n${userContext}\n=== END USER PROFILE ===` : '', dealContext ? `\n\nDEAL CONTEXT:\n${dealContext}` : '', knowledgeContext ? `\n\nRELEVANT COACHING KNOWLEDGE (use as YOUR expertise):\n${knowledgeContext.substring(0, 3000)}` : ''].filter(Boolean).join('\n')
  ...
  const stream = anthropic.messages.stream({ model: 'claude-sonnet-4-6', max_tokens: 1024, system: fullSystemPrompt, messages })
